(() => {
  ...
  const APP_VERSION = "20260604-clean-warrior-95";
  ...
  class SoulriftGame {
    ...
    updatePowerDomain(effect, dt) {
      ...
      if (localCasterInside) {
        if (kind === "nature") this.healPlayer(1.0, { source: "power" });
        if (kind === "blood") this.healPlayer(0.75, { source: "power" });
        if (kind === "time") {
    ...
    drawHero(ctx, x, y, scale, actor, power, custom) {
      ...
      ctx.fillStyle = "#0f131d";
      roundPixel(ctx, -17 - recoilFrame * 2, leftArmY - 1, 9, 17, 2);
      roundPixel(ctx, 8 + hitFrame * 2, rightArmY - 1, 9, 17, 2);
      ctx.fillStyle = armorDark;
      roundPixel(ctx, -15 - recoilFrame * 2, leftArmY, 6, 15, 2);
      roundPixel(ctx, 10 + hitFrame * 2, rightArmY, 6, 15, 2);
      ctx.fillStyle = "#0f131d";
      roundPixel(ctx, -13, -14 - holdFrame, 26, 27, 5);
      ctx.fillStyle = armorCore;
      roundPixel(ctx, -11, -12 - holdFrame, 22, 23, 4);
      roundPixel(ctx, -16, -10 - holdFrame, 8, 7, 2);
      roundPixel(ctx, 8, -10 - holdFrame, 8, 7, 2);
